...
- [ ] JWT verification pins `alg`/`typ`, rejects oversized tokens, and compares signatures without early exit.
- [ ] JWT payloads include a unique token id so refresh rotation changes the stored refresh hash even within the same second.
...
- [ ] Refresh token reuse revokes the affected session and writes a privacy-safe audit event.
