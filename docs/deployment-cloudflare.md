...
`worker:smoke:local` discovers every SQL file in `services/api-worker/migrations`, records applied files in the local D1 `schema_migrations` ledger, applies only pending migrations in filename order, and verifies that every discovered migration has a ledger entry before it starts the local Worker. Use it as the default local runtime gate instead of maintaining a separate hand-run migration list.
