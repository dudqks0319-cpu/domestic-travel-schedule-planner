...
`worker:smoke:local` is the preferred local runtime gate. It discovers every SQL file in `services/api-worker/migrations`, applies them to local D1 in filename order, starts `wrangler dev --local`, runs the full v1 write smoke, and stops the Worker process.
